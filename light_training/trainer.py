    ...
    def validate(self):
        ...
        if self.global_step % self.val_every == 0 \
            ...
            for i in range(len(self.val_loader)):
                ...
                with torch.no_grad():
                    with torch.autocast("cuda", enabled=False) if (self.ddp or 'cuda' in self.device) else dummy_context():
    ...
    def train_epoch(self, 
        ...
        for i in range(self.num_step_per_epoch):
            ...
            if not self.auto_optim:
                ...
                with autocast("cuda", enabled=False) if (self.ddp or 'cuda' in self.device) else dummy_context():
                    loss = self.training_step(batch)
        ...
        if self.local_rank==0:
            mean_loss = np.mean(epoch_loss_values)
            print(f'Avg Training loss at Epoch:{epoch}::: {mean_loss}')
            self.log("epoch_loss", mean_loss, epoch)
            e = time.time()
            print(f"epoch time is {e - s}")
